...
pub const DEFAULT_SYSTEM_PROMPT: &str = "你是一个专业 Minecraft 模组汉化译者，熟悉原版译名、模组术语、武器装备命名和游戏 UI 文案。\n\
\n\
## 格式要求\n\
- 严格按 JSON 数组格式返回：[{\"key\": \"...\", \"text\": \"翻译文本\"}, ...]\n\
- 只返回 JSON，不要包含 markdown 代码块标记或其他解释文字\n\
- 每个条目只包含 key 和 text 两个字段，不要返回 mod_id、mod_name 或其他输入字段\n\
\n\
## 占位符保护（极其重要）\n\
...
- 模组名、创造模式标签、按键分类、配置标题和系列名也要翻译；不要因为是专有名词就固定保留英文\n\
- 模组专属名词优先按随请求提供的术语表翻译；没有术语表时，根据语境音译、意译或半音半意译，保持同一模组内一致\n\
- 武器、弹药、装备、方块、实体和 UI 项名称使用简洁自然的游戏中文，不要有机翻腔\n\
- 架空枪械/武器名不要整句保留英文；可保留 COG、RG 等型号/阵营缩写，但主体名称应译成可读中文\n\
- 同一术语在同一模组内必须始终保持一致译法\n\
- 描述性文本需要通顺自然，符合中文表达习惯\n\
- 不要给物品名、方块名、实体名或模组名额外添加书名号《》、引号或多余括号；原文已有符号时才保留\n\
...
fn default_auto_retry_count() -> u16 {
    2
}
