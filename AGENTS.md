...
- 更新 README 时默认同步维护 `README.md` 和 `README.en-US.md`，除非用户明确要求只改其中一份。
- README 文案语气保持克制、准确、偏功能说明和工程说明；避免广告式、推销式或过度夸大的表达。
- 不使用缺乏实测依据的强宣传词，例如“秒级”“极致”“越用越准”“一键解决”“最佳”等；如果确实需要性能描述，应写明可验证的条件或截图中的实际数据。
- 功能小标题不要直接照搬用户需求关键词，应润色成自然、稳定的文档标题，但不得改变功能含义或夸大能力边界。
- 中英文 README 的章节结构、截图引用、功能覆盖范围和重要限制应保持一致；英文不是逐字硬译，但信息量必须对齐。
- 新增截图时优先放在 `docs/readme/screenshots/`，控制单张图片体积和 README 展示尺寸，避免页面被大图刷屏。
